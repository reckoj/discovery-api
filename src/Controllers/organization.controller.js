...
exports.add = async (req, res, next) => {
    try {
        const validatedData = await OrganizationValidator.validateAsync({ ...req.body });
        const query = {
            $or: [
        ...
        return res.status(HTTP_STATUS_CODE.CREATED).json({ status: HTTP_STATUS_CODE.CREATED, message: MESSAGE.USER_REGISTERED_SUCCESSFULLY, success: true, data: { organization } });
    } catch (error) {
...
exports.getAll = async (req, res, next) => {
    try {
        const { page = 1, limit = 10, island = '' } = req.query;
        const offset = (page - 1) * limit;

        const countQuery = Organization.countDocuments({ island: { $regex: island, $options: 'i' } });
        const query = Organization.find({ island: { $regex: island, $options: 'i' } }).skip(offset).limit(limit);

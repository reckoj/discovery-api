...
exports.getAll = async (req, res, next) => {
    try {
        ...
        const usersCount = await User.countDocuments({ user_type: { $ne: "super_admin" } });
...
exports.getAllOrg = async (req, res, next) => {
    try {
        ...
        const usersCount = await Customer.countDocuments({ user_type: { $ne: "super_admin" }, organization: name });
...
exports.getAllUser = async (req, res, next) => {
    try {
        ...
        const usersCount = await User.countDocuments({ user_type: "user", organization: name });
        const ticketsCount = await Tickets.countDocuments({ organization: name, raised_by: id });
        return res.status(HTTP_STATUS_CODE.OK).json({
            status: HTTP_STATUS_CODE.OK, message: MESSAGE.RESPONSE_SUCCESS, success: true, data: {
                tickets: ticketsCount,
                users: usersCount,
